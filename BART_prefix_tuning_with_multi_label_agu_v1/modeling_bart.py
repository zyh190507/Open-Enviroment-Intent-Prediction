...
class BartDecoder(BartPretrainedModel):
    ...
    def _prepare_decoder_attention_mask(self, attention_mask, input_shape, inputs_embeds, past_key_values_length):
        # create causal mask
        # [bsz, seq_len] -> [bsz, 1, tgt_seq_len, src_seq_len]
        # print(attention_mask.shape, input_shape)
...
class BartForConditionalGeneration(BartPretrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            if decoder_input_ids is None:
                if isinstance(labels, list):
                    decoder_input_list = []
                    for label in labels:
                        decoder_input_list.append(shift_tokens_right(label, self.config.pad_token_id,
        ...
        if isinstance(labels, list):
            total_loss = 0
            for decoder_input, label in zip(decoder_input_list, labels):
                # decoder outputs consists of (dec_features, past_key_value, dec_hidden, dec_attn)
                ...
                if labels is not None:
                    ...
                    masked_lm_loss = loss_fct(lm_logits.view(-1, self.config.vocab_size), label.view(-1))
                    total_loss += masked_lm_loss

            masked_lm_loss = total_loss / len(labels)
