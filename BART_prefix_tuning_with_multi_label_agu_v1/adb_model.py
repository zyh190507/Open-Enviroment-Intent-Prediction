...
from BART_prefix_tuning_with_multi_label_agu_v1.prefix_model import Model
from BART_prefix_tuning_with_multi_label_agu_v1.pipe import Pipe, set_seed
from BART_prefix_tuning_with_multi_label_agu_v1.metric import BoundaryLoss, F_measure, euclidean_metric
from BART_prefix_tuning_with_multi_label_agu_v1.utils import load_parameters, pack_labels_batch
...
class ModelManager:
    ...
    def evaluation(self, args, data, mode="eval"):
        ...
        for batch in tqdm(dataloader, desc="Iteration"):
            ...
            labels = [label.to(self.device) for label in labels][0]
            batch = tuple(t.to(self.device) for _, t in batch.items())
            input_ids, input_mask, multi_labels, label_ids = batch
            with torch.set_grad_enabled(False):
                pooled_output = self.model(input_ids=input_ids, attention_mask=input_mask,
        ...
        if mode == 'eval':
            ...
            unk_path = "unkdataset-{}-known_cls_ratio-{}-seed-{}-lr-{}-pre_seq_len-{}-lmd-{}-delta-{}.pkl".format(
                args.dataset, args.known_cls_ratio,
                args.seed, args.lr, args.pre_seq_len, args.lamda_loss, args.delta)
    ...
    def save_results(self, args):
        ...
        var = [args.dataset, args.known_cls_ratio, args.labeled_ratio, args.seed, args.pre_seq_len,
               args.lr, args.lamda_loss, args.m_loss]
        names = ['dataset', 'known_cls_ratio', 'labeled_ratio', 'seed', 'psl', 'lr', 'lmd', 'm_loss']
