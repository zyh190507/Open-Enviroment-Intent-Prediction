...
from BART_full_finetune.pipe import Data
...
class GenerateOOD:
    ...
    def _generate_utterance(self):
        ...
        arg = self.args
        pickle_name = "dataset-{}-known_cls_ratio-{}-seed-{}-lr-{}".format(
        ...
        if os.path.exists(os.path.join(file_path, "data.pkl")):
            ...
            for batch in tqdm(dl, desc="Iteration"):
                ...
                generate_tokens = self.model.evaluate_step(input_ids=input_ids, attention_mask=input_mask, labels=labels)
...
if __name__ == '__main__':
    ...
    from BART_full_finetune.pipe import set_seed
    ...
    args.p_node = 0.15
    gen_ood = GenerateOOD(args, data, unkDs, pretrain_model_path, tokenizer_path, rama_cluster_times=2)
