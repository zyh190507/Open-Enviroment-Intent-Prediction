...
from GPT2_prefix_tuning_with_multi_label_v2.metric import BoundaryLoss, F_measure, euclidean_metric
...
class ModelManager:
    ...
    def evaluation(self, args, data, mode="eval"):
        ...
        for batch in tqdm(dataloader, desc="Iteration"):
            ...
            label_ids = batch.pop("label_ids").to(self.device)
            if mode == 'test':
                multi_labels = batch.pop("multi_labels")
                batch = tuple(pack_labels_batch(t, self.device)[0] for _, t in batch.items())
                input_ids, input_mask, x_attention_mask, labels = batch
            else:
                batch = tuple(pack_labels_batch(t, self.device)[0] for _, t in batch.items())
                input_ids, input_mask, multi_labels, x_attention_mask, labels = batch
            with torch.set_grad_enabled(False):
                ...
                if mode == 'test':
                    for input_id, attention_mask, per_label, pred_id, x_attention, label_id in zip(
                            input_ids.cpu().numpy(),
        ...
        if mode == 'eval':
            ...
            unk_path = "unkdataset-{}-known_cls_ratio-{}-seed-{}-lr-{}-pre_seq_len-{}-lmd-{}-m-loss-{}.pkl".format(
    ...
    def train(self, args, data):
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(data.train_dataloader, desc="Iteration")):
                batch.pop("label_text")
                label_ids = batch.pop("label_ids").to(self.device)
                batch = tuple(pack_labels_batch(t, self.device) for _, t in batch.items())
                input_ids, input_mask, multi_labels, x_attention_mask, labels = batch
                with torch.set_grad_enabled(True):
                    features = self.model.forward(input_ids=input_ids, attention_mask=input_mask, multi_labels=multi_labels,
                    ...
                    nb_tr_examples += input_ids[0].size(0)
                    nb_tr_steps += 1
    ...
    def centroids_cal(self, args, data):
        centroids = torch.zeros(args.adb_num_labels, args.feat_dim).cuda()
        total_labels = torch.empty(0, dtype=torch.long).to(self.device)
        ...
        with torch.set_grad_enabled(False):
            for batch in data.train_dataloader:
                ...
                features = self.model(input_ids=input_ids, attention_mask=input_mask, labels=labels,
                                      x_attention_mask=x_attention_mask, multi_labels=multi_labels,
    ...
    def save_results(self, args):
        ...
        var = [args.dataset, args.known_cls_ratio, args.labeled_ratio, args.seed, args.lr, args.pre_seq_len,
               args.lamda_loss, args.m_loss]
        names = ['dataset', 'known_cls_ratio', 'labeled_ratio', 'seed', 'lr', 'psl', 'lmd', 'm_loss']
