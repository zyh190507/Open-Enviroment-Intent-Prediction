...
from fastNLP import DataSet, TorchDataLoader
...
from BART_prefix_tuning.pipe import Data
...
class GenerateOOD:
    ...
    def _generate_utterance(self):
        ...
        arg = self.args
        pickle_name = "dataset-{}-known_cls_ratio-{}-seed-{}-lr-{}-lmd-{}".format(
            arg.dataset, arg.known_cls_ratio,
            arg.seed, arg.lr, args.lamda_loss)
        ...
        if os.path.exists(os.path.join(file_path, "data.pkl")):
            ...
            for batch in tqdm(dl, desc="Iteration"):
                ...
                for pred_word in decoded_preds:
                    num_beams.append(pred_word.replace("It", "").replace("was", "").strip())
                    if len(num_beams) == 3:
            ...
            #  保存生成的文件
            # df_gen.to_csv("{}-lr-{}-seed-{}_gen_result.csv".format(self.args.dataset, arg.lr, args.seed))
            # 保存文件 pickle 文件
...
if __name__ == '__main__':
    ...
    from BART_prefix_tuning.pipe import set_seed
