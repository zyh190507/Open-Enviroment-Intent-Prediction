...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, mode):
    ...
    for (ex_index, example) in enumerate(examples):
        max_seq_length = 128
        # label 的监督信号， 长度为 16（最大标签不会超过 16 个词）
        template_text = "{} </s>".format(example.label.replace("_", " ").replace("-", " "))
        ...
        src_input_ids = inputs["input_ids"].squeeze()
        src_attention_mask = inputs["attention_mask"].squeeze()
        # 将所有的 pad 改为 -100 以便计算交叉熵时候忽略掉
        labels = tg_inputs["input_ids"].squeeze()
        labels[labels == tokenizer.pad_token_id] = -100

        label_text = example.label.replace("_", " ").replace("-", " ")
        ...
        if ex_index < 5:
            ...
            # print("tg_input_ids: %s" % " ".join([str(x) for x in tg_input_ids.numpy()]))
            # print("tg_input_mask: %s" % " ".join([str(x) for x in tg_attention_mask.numpy()]))
