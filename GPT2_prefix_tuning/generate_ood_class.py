...
from GPT2_prefix_tuning.pipe import Data
...
class GenerateOOD:
    ...
    def __init__(self, args, data: Data, unkdataset_or_filename, pretrain_model_or_path, tokenizer_or_path,
        ...
        if isinstance(pretrain_model_or_path, str):
            ...
            model_filepath = "model/dataset-{}-known_cls_ratio-{}-seed-{}-lr-{}-pre_seq_len-{}-prefix_projection-{}" \
                             "-prefix_hidden_size-{}".format(
                args.dataset, args.known_cls_ratio,
                args.seed, args.lr, args.pre_seq_len, args.prefix_projection, args.prefix_hidden_size)
    ...
    def _generate_utterance(self):
        ...
        arg = self.args
        pickle_name = "dataset-{}-known_cls_ratio-{}-seed-{}-lr-{}-pre_seq_len-{}-prefix_projection-{}" \
                      "-prefix_hidden_size-{}-prefix_dropout-{}-train_batch_size-{}".format(
            arg.dataset, arg.known_cls_ratio,
            arg.seed, arg.lr, arg.pre_seq_len, arg.prefix_projection, arg.prefix_hidden_size, arg.prefix_dropout,
        ...
        if os.path.exists(os.path.join(file_path, "data.pkl")):
            ...
            for batch in tqdm(dl, desc="Iteration"):
                # batch = {n: t.to(self.device) for n, t in batch.items()}
                input_ids, input_mask, label_ids, labels = batch['input_ids'].to(self.device), batch[
                    ...
                    self.device)
                x_attention_mask = batch['x_attention_mask'].to(self.device)
                # 收集真实标签
                ...
                for pred_word in decoded_preds:
                    num_beams.append(pred_word.split("It")[0].strip())
                    if len(num_beams) == 3:
...
if __name__ == '__main__':
    from GPT2_prefix_tuning.utils import load_parameters, get_unkdataset_of_dataset
    from GPT2_prefix_tuning.pipe import set_seed
    from GPT2_prefix_tuning.prefix_model import Model
    from fastNLP import cache_results

    args = load_parameters()

    datafile = "datafile/dataset-{}-known_cls_ratio-{}-seed-{}-lr-{}-pre_seq_len-{}-prefix_projection-{}" \
               "-prefix_hidden_size-{}".format(
        args.dataset, args.known_cls_ratio,
        args.seed, args.lr, args.pre_seq_len, args.prefix_projection, args.prefix_hidden_size)
    ...
    unkDs = "unkdataset_with_noise/unkdataset-{}-known_cls_ratio-{}-seed-{}-lr-{}-pre_seq_len-{}-prefix_projection-{}.pkl".format(
                args.dataset, args.known_cls_ratio,
                args.seed, args.lr, args.pre_seq_len, args.prefix_projection, args.prefix_hidden_size)
    ...
    gen_ood = GenerateOOD(args, data, unkDs, pretrain_model_path, tokenizer_path, rama_cluster_times=8)
