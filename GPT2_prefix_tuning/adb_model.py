...
from GPT2_prefix_tuning.pipe import Data, set_seed
from GPT2_prefix_tuning.metric import BoundaryLoss, F_measure, euclidean_metric
from GPT2_prefix_tuning.utils import load_parameters
...
class ModelManager:
    ...
    def evaluation(self, args, data, mode="eval"):
        ...
        if mode == 'eval':
            ...
            unk_path = "unkdataset-{}-known_cls_ratio-{}-seed-{}-lr-{}-pre_seq_len-{}-prefix_projection-{}.pkl".format(
            ...
            for step, batch in enumerate(tqdm(data.train_dataloader, desc="Iteration")):
                ...
                with torch.set_grad_enabled(True):
                    features = self.model.forward(input_ids=input_ids, attention_mask=input_mask,
                                                  labels=None, x_attention_mask=x_attention_mask, label_ids=None)[
                        'feature']
    ...
    def centroids_cal(self, args, data):
        ...
        with torch.set_grad_enabled(False):
            for batch in data.train_dataloader:
                ...
                batch = tuple(t.to(self.device) for _, t in batch.items())
                input_ids, input_mask, x_attention_mask, labels, label_ids = batch
                features = self.model(input_ids=input_ids, attention_mask=input_mask, labels=None,
    ...
    def save_results(self, args):
        ...
        names = ['dataset', 'known_cls_ratio', 'labeled_ratio', 'seed', 'lr', 'psl']
